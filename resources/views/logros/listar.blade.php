...
<body>
    <div class="container">
        ...
        <table class="custom-table"> <!-- Agregamos una clase para la tabla -->
            ...
            @foreach ($achiviements as $achiviement)
            {{-- creo una fila --}}
            <tr>
                <td>{{$achiviement->id}}</td>
                <td>{{$achiviement->name}}</td>
                <td>{{$achiviement->description}}</td>
                <td>{{$achiviement->prize}}</td>
                <!-- Usamos iconos de FontAwesome para mostrar y eliminar -->
                <td><a href="{{route('logro.show', $achiviement->id)}}" class="button"><i class="fas fa-eye"></i></a></td>
                <td><a href="{{ route('achiviement.edit', $achiviement->id) }}" class="button"><i class="fa-solid fa-pen"></i></a>
                </td>

                <form action="{{route('logro.destroy',$achiviement->id)}}" method="POST">
                    @csrf
                    @method('delete')
